...
pub trait RttStore: Send + Sync {
    ...
    ///
    /// Called automatically by `quic-proto` when a client connection drains after the handshake
    /// has completed.
    fn insert(&self, server_name: &str, rtt_us: u64);

    /// Retrieve the previously stored SRTT (in microseconds) for `server_name`, if any.
    ///
    /// Called automatically by `quic-proto` when creating a new outbound connection.
...
///
/// Configure this with [`ClientConfig::rtt_store`](crate::ClientConfig::rtt_store) to cache values
/// for the lifetime of the process. Use a custom [`RttStore`] implementation if you need
...
impl RttStore for RttMemoryCache {
    ...
    fn insert(&self, server_name: &str, rtt_us: u64) {
        self.0
            ...
            .insert(server_name.into(), rtt_us);
    }
